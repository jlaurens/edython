...
ezP.Delegate.prototype.getInputTuple_ = function(block, name) {
  ...
  while ((input = list[i])) {
    ...
    if (end == start+1) {
      c8n = block.makeConnection_(Blockly.INPUT_VALUE);
      input = new Blockly.Input(Blockly.INPUT_VALUE,'S7R_'+grp+'_1', block, c8n);
      tuple = input.ezpTuple = {grp: grp, n: 0, sep: tuple.sep, isSeparator:true};
      list.splice(end,0,input);
      ++end;
      c8n = block.makeConnection_(Blockly.INPUT_VALUE);
      input = new Blockly.Input(Blockly.INPUT_VALUE,'S7R_'+grp+'_'+n, block, c8n);
