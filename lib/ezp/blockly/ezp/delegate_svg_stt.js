...
ezP.DelegateSvg.Manager.register(ezP.Const.stt.ANY, ezP.DelegateSvg.Statement);
  ...
  for (var i = 0, input; input = block.inputList[i]; i++) {
    for (var j = 0, field; field = input.fieldRow[j]; j++) {
      if (field instanceof ezP.FieldPrintOptions) {
        return this.printState_ = field.getState_();
      }
    }
  }
...
ezP.DelegateSvg.Statement.Print.prototype.toDom = function(block, element) {
  ...
  f(ezP.FieldPrintOptions.END);
  f(ezP.FieldPrintOptions.SEP);
  f(ezP.FieldPrintOptions.FILE);
  f(ezP.FieldPrintOptions.FLUSH);
  if (l.length) {
