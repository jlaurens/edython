...
EZP.FieldVariable.prototype.onItemSelected = function(menu, menuItem) {
  ...
  if (this.sourceBlock_ && this.sourceBlock_.workspace) {
    ...
    if (variable) {
      ...
      return;
    } else if (id == EZP.NEW_VARIABLE_ID) {
      // Create new variable.
      var me = this;
      itemText = Blockly.Variables.createVariable(workspace, function(text) {
        me.setValue(text);
      });
      return;
    }
...
EZP.FieldVariable.prototype.createMenu_ = function() {
  ...
  content = Blockly.Msg.NEW_VARIABLE;
  value = EZP.NEW_VARIABLE_ID;
  renderer = EZP.MenuItemRenderer.getInstance();
  menuItem = new goog.ui.MenuItem(content, undefined, undefined, renderer);
  menuItem.setRightToLeft(this.sourceBlock_.RTL);
  menuItem.setValue(value);
