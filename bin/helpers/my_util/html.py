...
'''
  ...
  def deps(path_or_list, root):
    ans = ''
    try:
      ans = ''.join(f'''  <script src="{root}{x}"></script>
''' for x in path_or_list)
    except:
      ans = path_or_list.read_text()
...
'''
  ...
  def test(basename, test=False):
    test = '.test' if test else ''
    return f'''{HTML.script(f'./{basename}{test}.js')}'''
