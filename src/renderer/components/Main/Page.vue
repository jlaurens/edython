<template>
  ...
  >
    ...
    >
      <div>
        <div
          style="display:inline-block;width:calc(64px + 1rem);vertical-align:top;"
        >
          <img
          ...
          style="display:inline-block;width:calc(100% - 64px - 1rem);"
        >
          {{ $$t('panel.launch.content') }}
        </div>
      </div>
    </b-modal>
  </div>
...
import BrickToolbar from '@@/Toolbar/Brick'
...
export default {
    ...
    components: {
        ...
        PaneConsole1,
        PaneTurtle,
        BrickToolbar, // eslint-disable-line 
        Console2Script // eslint-disable-line 
    },
    data () {
        return {
            footstep: 0,
    ...
    watch: {
        toolbarBlockVisible (newValue, oldValue) { // eslint-disable-line no-unused-vars
    ...
    methods: {
        ...
        onDragSplit (size) { // eslint-disable-line no-unused-vars
            this.$root.$emit('toolbar-follow-phantom')
        },
        where (what) {
