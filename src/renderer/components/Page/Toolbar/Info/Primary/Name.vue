<template>
  <b-button-group :id="id" class="mx-1">
    <b-form-input v-model="myName" type="text" class="btn btn-outline-secondary eyo-form-input-text" :style='{fontFamily: $$.eYo.Font.familyMono}'></b-form-input>
    <b-dropdown :id="'dd-' + id" class="eyo-dropdown" v-if="module" variant="outline-secondary">
      <b-dropdown-item-button v-for="method in methods" v-on:click="myName = method" :key="method" class="eyo-info-primary-variant eyo-code">{{method}}</b-dropdown-item-button>
    </b-dropdown>
  </b-button-group>
</template>

<script>
  export default {
    name: 'info-primary-name',
    data () {
      return {
        myName_: undefined,
        methods_: undefined
      }
    },
    props: {
      eyo: {
        type: Object,
        default: undefined
      },
      name: {
        type: String,
        default: undefined
      },
      variant: {
        type: String,
        default: undefined
      },
      module: {
        type: Object,
        defaut: undefined
      }
    },
    computed: {
      id () {
        return 'eyo-info-name'
      },
      myName: {
        get () {
          return this.name
        },
        set (newValue) {
          this.eyo.name_p = newValue
        }
      },
      methods: {
        get () {
          //
          var ra = []
          this.module.forEachItemWithType(this.variant === eYo.Key.CALL_EXPR ? 'function' : 'data', item => {
            ra.push(item.name)
          })
          return ra
        }
      }
    }
  }
</script>
<style>
#dd-info-primary-name > .btn {
  padding-left:0.25rem;
}
.dropdown-menu {
    height: auto;
    max-height: 400px;
    overflow-x: hidden;
}
</style>
