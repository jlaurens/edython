describe('Wrapped slots', function() {
  ;[
    ...
    it (`Slot ${k}`, function () {
      var d = eYo.Test.new_dlgt('assignment_stmt')
...
describe('Each assignment block type', function() {
  [
    ...
    it (`basic operator: ${Ts[0]} / ${Ts[1] || Ts[0]} / ${Ts[2]}`, function () {
      ...
      eYo.Test.ctor(d, 'assignment_stmt')
...
describe('Assignment', function() {
  ...
  it('variant change', function() {
    eYo.Test.setItUp()
    var d1 = eYo.Test.new_dlgt('expression_stmt')
    eYo.Test.ctor(d1, 'assignment_stmt')
  ...
  it('Expression only', function() {
    ...
    chai.assert(d1.value_t.lastConnect(d2), 'MISSED C8N 1')
    chai.assert(input.connection.targetBlock() === d2, 'MISSED C8N 2')
    d1.block_.dispose()
    eYo.Test.tearItDown()
  })
  ...
  it ('Annotated defined and dom', function () {
    eYo.Test.setItUp()
    var d1 = eYo.Test.new_dlgt('annotated_assignment_stmt')
    var dom = eYo.Xml.blockToDom(d1)
    // console.error(dom)
    d1.block_.dispose()
    d1 = eYo.Test.new_dlgt(dom)
    d1.block_.dispose()
    eYo.Test.tearItDown()
  })
})
describe('TEST', function () {
  it ('Annotated alone', function () {
    ...
    dom = eYo.Xml.blockToDom(d1)
    console.error(dom)
    d1.block_.dispose()
    d1 = eYo.Test.new_dlgt(dom)
    chai.assert(d1.value_s.isIncog(), 'UNEXPECTED VALUE SLOT')
    d1.block_.dispose()
    eYo.Test.tearItDown()
  })
})
describe('T&ST', function () {
  ...
  it ('augtarget_annotated in target slot', function () {
    ...
    d2.target_t.lastConnect(eYo.Test.new_dlgt('identifier'))
    d2.target_s.unwrappedTarget.variant_p = eYo.Key.SLICING
    eYo.Test.dlgt('augtarget_annotated')
    eYo.Test.list_connect(d1, 'target', d2)
    eYo.Test.dlgt(d1, 'annotated_assignment_stmt')
    eYo.Test.variant(d1, 'TARGET_VALUED') // No 2 annotations
    eYo.Test.incog(d1, ['target', 'Xannotated', 'value'])
...
describe('Copy/Paste', function() {
  ...
  it ('identifier_annotated in name', function () {
    ...
    chai.assert(d1.target_t.lastConnect(d2), `MISSED C8N 1`)
    chai.assert(d1.target_s.unwrappedTarget === d2, `MISSED C8N 2`)
    eYo.Test.variant(d1, 'TARGET_VALUED')
    var dom = eYo.Xml.blockToDom(d1)
    // console.error(dom)
    d1.block_.dispose()
    d1 = eYo.Test.new_dlgt(dom)
    chai.assert(!d1.value_s.isIncog(), 'MISSING SLOT')
    d1.block_.dispose()
    eYo.Test.tearItDown()
  })
})
...
describe('One block: assignment_stmt', function() {
  it('target', function() {
    ...
    d1.target_t.lastConnect(d2)
    eYo.Test.input_length(d1.target_b, 3, `MISSED C8N 1`)
    chai.assert(d1.target_s.unwrappedTarget === d2, `MISSED C8N 2`)
    ...
    d3.will_remain = true
    eYo.STOP = true
    chai.assert(!d1.target_t.lastConnect(d3), 'Connection is expected failed.')
    eYo.Test.input_length(d1.target_b, 3, `MISSED C8N 2`)
    d3.block_.dispose()
    d1.block_.dispose()
    eYo.Test.tearItDown()
  })
...
describe('One block: annotated_stmt', function() {
  it('target', function() {
    ...
    chai.assert(d1.annotated_b === d3, 'MISSED C8N')
...
describe('One block: annotated_assignment_stmt', function() {
  it ('target is annotated', function () {
    eYo.Test.setItUp()
    var main = eYo.Test.new_dlgt('assignment_stmt')
    var annotated = eYo.Test.new_dlgt({
      type: eYo.T3.Expr.primary,
      target_p: 'x',
      annotated_p: 'str'
    })
    eYo.Test.list_connect(main, 'target', annotated)
    eYo.Test.dlgt(main, 'annotated_assignment_stmt')
    annotated.block_.dispose()
    eYo.Test.dlgt(main, 'assignment_stmt')
    main.dispose()
    eYo.Test.tearItDown()
  })
  it('target', function() {
    eYo.Test.setItUp()
    var main = eYo.Test.new_dlgt('annotated_assignment_stmt')
    chai.assert(main.operator_p === '=', `MISSED ${main.operator_p}`)
    eYo.Test.code(main, '<MISSING NAME>:<MISSING EXPRESSION>=<MISSING EXPRESSION>')
    var d2 = eYo.Test.new_dlgt('x')
    chai.assert(d2.target_p === 'x', 'MISSED 1')
    main.target_t.lastConnect(d2)
    eYo.Test.input_length(main.target_b, 1, `MISSED C8N 1`)
    main.annotated_p = 'fou + bar'
    chai.assert(main.annotated_p === 'fou + bar', 'MISSED ANNOTATION')
    chai.assert(main.annotated_s.bindField.visible_, 'UNEXPECTED HIDDEN')
    chai.assert(main.annotated_s.bindField.getText() === 'fou + bar', 'MISSED VALUE')
    console.error(main.annotated_s)
    ...
    var d3 =  eYo.Test.new_dlgt(`<x eyo="a_expr" operator="+" xmlns="urn:edython:0.2" xmlns:eyo="urn:edython:0.2"><x eyo="identifier" name="abc" slot="lhs"></x><x eyo="identifier" name="bcd" slot="rhs"></x></x>`)
    eYo.Test.code(d3, 'abc + bcd')
    main.annotated_s.connect(d3)
    chai.assert(main.annotated_b === d3, 'MISSED C8N')
    chai.assert(!main.annotated_s.bindField.visible_, 'UNEXPECTED VISIBLE')
    eYo.Test.code(main, 'xxx: abc + bcd=<MISSING EXPRESSION>')
    // then replace the target block with an annotated identifier
    d3 = eYo.Test.new_dlgt('Z')
    d3.variant_p = eYo.Key.ANNOTATED
    d3.annotated_p = 'abcd + cdef'
    eYo.Test.code(d3, 'Z: abcd + cdef')
    eYo.Test.dlgt(d3, 'identifier_annotated')
    // it's a unique target
    chai.assert(main.target_b.inputList[0].eyo.connect(d3), 'MISSED C8N 4')
    eYo.Test.dlgt(main, 'annotated_assignment_stmt')
    eYo.Test.input_length(main.target_b, 1, `MISSED C8N 5`)
    var eyo = main.target_s.unwrappedTarget
    chai.assert(eyo === d3, 'MISSED C8N 7')
    console.error(main.toLinearString)
    eYo.Test.code(d3, 'xxx: abcd + cdef')
    d3.block_.dispose() // orphan recovery: the old d2 block has been reconnected into d3
    eYo.Test.dlgt(main, 'assignment_stmt')
...
describe('Copy/Paste with value', function() {
  var f = t => {
    it(`basic ${t}`, function() {
      ...
      chai.assert(d.value_t.lastConnect(dd))
      var dom = eYo.Xml.blockToDom(d)
      // console.log(dom)
      chai.assert(dom.tagName.toLowerCase() === 's')
      dd = eYo.Test.new_dlgt(dom)
