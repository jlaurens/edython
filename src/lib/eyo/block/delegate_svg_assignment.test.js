describe('Each assignment block type', function() {
  ['expression_stmt',
    ...
    it (`basic type: ${t}`, function () {
      ...
      eYo.Test.assert_ctor(b, 'assignment_stmt')
      b.dispose()
    })
  })
})
...
describe('Assignment', function() {
  var f = (t, incogs, variant, comment_variant) => {
    it(t, function() {
      var b = eYo.Test.new_block(t)
      eYo.Test.assert_ctor(b, 'assignment_stmt')
      eYo.Test.assert_block(b, t)
      eYo.Test.assert_variant(b, variant)
      eYo.Test.assert_comment_variant(b, comment_variant)
      eYo.Test.assert_incog(b, incogs)
      b.dispose()
    })  
  }
  ...
  it('comment variant change', function() {
    ...
    eYo.Test.assert_ctor(b1, 'assignment_stmt')
    eYo.Test.assert_variant(b1, 'NONE')
    eYo.Test.assert_comment_variant(b1, 'COMMENT')
    eYo.Test.assert_incog(b1, ['Xtarget', 'Xannotated', 'Xvalue', 'comment'])
    b1.eyo.comment_variant_p = eYo.Key.NONE
    eYo.Test.assert_comment_variant(b1, 'NONE', '2')
    eYo.Test.assert_variant(b1, 'VALUED', '2')
    eYo.Test.assert_incog(b1, ['Xtarget', 'Xannotated', 'value', 'Xcomment'])
    b1.eyo.variant_p = eYo.Key.NONE
    eYo.Test.assert_variant(b1, 'NONE', '3')
    eYo.Test.assert_comment_variant(b1, 'COMMENT', '3')
    eYo.Test.assert_incog(b1, ['Xtarget', 'Xannotated', 'Xvalue', 'comment'])
    b1.dispose()
  })  
  it('variant change', function() {
    var b1 = eYo.Test.new_block('expression_stmt')
    eYo.Test.assert_ctor(b1, 'assignment_stmt')
    eYo.Test.assert_block(b1, 'expression_stmt')
    eYo.Test.assert_variant(b1, 'NONE')
    eYo.Test.assert_comment_variant(b1, 'COMMENT', '1')
    eYo.Test.assert_incog(b1, ['Xtarget', 'Xannotated', 'Xvalue', 'comment'])
    b1.eyo.comment_variant_p = eYo.Key.NONE
    eYo.Test.assert_comment_variant(b1, 'NONE', '2')
    eYo.Test.assert_variant(b1, 'VALUED', '2')
    ...
    var f = (v, target, annotation, value, comment, str) => {
      ...
      eYo.Test.assert_variant(b1, v, str)
      eYo.Test.assert_incog(b1, target, annotation, value, comment)
    }
  ...
  it('Expression only', function() {
    ...
    eYo.Test.assert_variant(b1, 'TARGET_VALUED')
    var b2 = eYo.DelegateSvg.newBlockReady(Blockly.mainWorkspace, eYo.T3.Expr.assignment_chain)
    eYo.Test.assert_block(b2, 'identifier_valued')
    var input = b1.eyo.value_t.eyo.lastInput
    chai.assert(b1.eyo.value_t.eyo.lastConnect(b2), 'MISSED C8N 1')
  ...
  it ('Annotated defined and dom', function () {
    ...
    eYo.Test.assert_block(b1, 'annotated_assignment_stmt', `dom: ${dom}`)
    b1.dispose()
  })
  it ('Annotated alone', function () {
    var b1 = eYo.Test.new_block('annotated_stmt')
    eYo.Test.assert_variant(b1, 'ANNOTATED')    
    dom = eYo.Xml.blockToDom(b1)
    console.error(dom)
    b1.dispose()
    b1 = eYo.DelegateSvg.newBlockReady(Blockly.mainWorkspace, dom)
    eYo.Test.assert_block(b1, 'annotated_stmt', 'dom')
  ...
  it ('identifier_annotated in target slot', function () {
    ...
    eYo.Test.assert_list_connect(b1, 'target', b2)
    eYo.Test.assert_block(b1, 'annotated_assignment_stmt')
    eYo.Test.assert_variant(b1, 'TARGET_VALUED') // No 2 annotations
    eYo.Test.assert_incog(b1, ['target', 'Xannotated', 'value', 'Xcomment'])
    var dom = eYo.Xml.blockToDom(b1)
    // console.error(dom)
    var b3 = eYo.DelegateSvg.newBlockReady(Blockly.mainWorkspace, dom)
    eYo.Test.assert_block(b3, 'annotated_assignment_stmt', `dom: BLOCK ${dom}`)
    eYo.Test.assert_incog(b3, ['target', 'Xannotated', 'value', 'Xcomment'])
    b3.dispose()
    b2.dispose()
    eYo.Test.assert_block(b1, 'assignment_stmt')
    eYo.Test.assert_variant(b1, 'TARGET_VALUED') // No 2 annotations
    eYo.Test.assert_incog(b1, ['target', 'Xannotated', 'value', 'Xcomment'])
    b1.dispose()
  })
  it ('augtarget_annotated in target slot', function () {
    ...
    b2.eyo.target_t.eyo.lastConnect(eYo.Test.new_block('identifier'))
    b2.eyo.target_s.unwrappedTarget.variant_p = eYo.Key.SLICING
    eYo.Test.assert_block('augtarget_annotated')
    eYo.Test.assert_list_connect(b1, 'target', b2)
    eYo.Test.assert_block(b1, 'annotated_assignment_stmt')
    eYo.Test.assert_variant(b1, 'TARGET_VALUED') // No 2 annotations
    eYo.Test.assert_incog(b1, ['target', 'Xannotated', 'value', 'Xcomment'])
...
describe('Comment/Variant changes', function() {
  it('comment variant change', function() {
    var b1 = eYo.Test.new_block('expression_stmt')
    eYo.Test.assert_ctor(b1, 'assignment_stmt')
    eYo.Test.assert_variant(b1, 'NONE', '1')
    eYo.Test.assert_comment_variant(b1, 'COMMENT', '1')
    eYo.Test.assert_incog(b1, ['Xtarget', 'Xannotated', 'Xvalue', 'comment'])
    b1.eyo.comment_variant_p = eYo.Key.NONE
    eYo.Test.assert_comment_variant(b1, 'NONE', '2')
...
describe('Copy/Paste', function() {
  ...
  it ('Annotated alone', function () {
    ...
    b1.dispose()
    b1 = eYo.DelegateSvg.newBlockReady(Blockly.mainWorkspace, dom)
    eYo.Test.assert_block(b1, 'annotated_stmt', `dom: ${dom}`)
    chai.assert(b1.eyo.value_s.isIncog(), 'UNEXPECTED VALUE SLOT')
    b1.dispose()
  })
  it ('identifier_annotated in name', function () {
    ...
    chai.assert(b1.eyo.target_t.eyo.lastConnect(b2), `MISSED C8N 1`)
    ...
    b1.dispose()
    b1 = eYo.DelegateSvg.newBlockReady(Blockly.mainWorkspace, dom)
    eYo.Test.assert_block(b1, 'annotated_assignment_stmt', `dom: BLOCK ${dom}`)
...
describe('One block: assignment_stmt', function() {
  it('target', function() {
    var b1 = eYo.Test.new_block('assignment_stmt')
    eYo.Test.assert_comment_variant(b1, 'NONE', '1')
    eYo.Test.assert_incog(b1, ['target', 'Xannotated', 'value', 'Xcomment'])
    b1.eyo.variant_p = eYo.Key.TARGET
    eYo.Test.assert_variant(b1, 'TARGET', '2')
    eYo.Test.assert_incog(b1, ['target', 'Xannotated', 'Xvalue', 'Xcomment'])
    ...
    eYo.Test.assert_input_length(b1.eyo.target_t, 3, `MISSED C8N 1`)
    chai.assert(b1.eyo.target_s.unwrappedTarget === b2.eyo, `MISSED C8N 2`)
    ...
    eYo.STOP = true
    chai.assert(!b1.eyo.target_t.eyo.lastConnect(b3), 'Connection is expected failed.')
    eYo.Test.assert_input_length(b1.eyo.target_t, 3, `MISSED C8N 2`)
    b3.dispose()
    b1.dispose()
  })
})
...
describe('One block: expression_stmt', function() {
  it('target', function() {
    ...
    eYo.Test.assert_incog(b1, ['Xtarget', 'Xannotated', 'Xvalue', 'comment'])
    b1.eyo.variant_p = eYo.Key.TARGET
    eYo.Test.assert_variant(b1, 'TARGET', '1')
    eYo.Test.assert_incog(b1, ['target', 'Xannotated', 'Xvalue', 'Xcomment'])
    eYo.Test.assert_block(b1, 'assignment_stmt')
    b1.eyo.target_p = 'abc'
    chai.assert(b1.eyo.target_p === 'abc', `MISSED ${b1.eyo.target_p} === 'abc'`)
    chai.assert(b1.eyo.target_t, 'MISSING TARGET')
    var b2 = eYo.DelegateSvg.newBlockReady(Blockly.mainWorkspace, 'de')
    eYo.Test.assert_input_length(b1.eyo.target_t, 1, `MISSED C8N 1`)
    b1.eyo.target_t.eyo.lastConnect(b2)
    ...
    eYo.Test.assert_block(b2, 'parent_module')
    eYo.Test.assert_input_length(b1.eyo.target_t, 1, `MISSED C8N 3`)
    b2.eyo.holder_s.connect(b3)
    eYo.Test.assert_block(b2, 'dotted_name')
    b1.eyo.target_t.eyo.lastConnect(b2)
    chai.assert(b1.eyo.target_s.unwrappedTarget === b2.eyo, 'MISSED CONNECTION 2')
    eYo.Test.assert_input_length(b1.eyo.target_t, 3, `MISSED C8N 4`)
    chai.assert(b2.eyo.holder_t === b3, `UW ${b2.eyo.holder_t.eyo.toString} === ${b3.eyo.toString}`)
    chai.assert(b1.eyo.target_s.unwrappedTarget === b2.eyo, 'MISSED CONNECTION 2')
    eYo.Test.assert_input_length(b1.eyo.target_t, 3, `MISSED C8N 5`)
    b1.dispose()
  })
})
...
describe('One block: annotated_stmt', function() {
  it('target', function() {
    var b1 = eYo.Test.new_block('annotated_stmt')
    chai.assert(b1.eyo.operator_p === '=', `MISSED ${b1.eyo.operator_p}`)
    eYo.Test.assert_comment_variant(b1, 'NONE')
    var b2 = eYo.DelegateSvg.newBlockReady(Blockly.mainWorkspace, 'x')
    eYo.Test.assert_block(b2, 'identifier')
    b1.eyo.target_t.eyo.lastConnect(b2)
    eYo.Test.assert_input_length(b1.eyo.target_t, 1, `1`)
    ...
    eYo.Test.assert_code(b1, 'x:fou+bar')
    var b3 =  eYo.DelegateSvg.newBlockReady(Blockly.mainWorkspace, `<x eyo="a_expr" operator="+" xmlns="urn:edython:0.2" xmlns:eyo="urn:edython:0.2"><x eyo="identifier" name="abc" slot="lhs"></x><x eyo="identifier" name="bcd" slot="rhs"></x></x>`)
    eYo.Test.assert_code(b3, 'abc+bcd')
    b1.eyo.annotated_s.connect(b3)
    chai.assert(b1.eyo.annotated_t === b3, 'MISSED C8N')
    chai.assert(!b1.eyo.annotated_s.bindField.visible_, 'UNEXPECTED VISIBLE')
    eYo.Test.assert_code(b1, 'x:abc+bcd')
    b1.dispose()
  })
})
...
describe('One block: annotated_assignment_stmt', function() {
  it('target', function() {
    ...
    var b2 = eYo.DelegateSvg.newBlockReady(Blockly.mainWorkspace, 'x')
    eYo.Test.assert_block(b2, 'identifier')
    chai.assert(b2.eyotarget_p === 'x', 'MISSED 1')
    b1.eyo.target_t.eyo.lastConnect(b2)
    eYo.Test.assert_input_length(b1.eyo.target_t, 1, `MISSED C8N 1`)
    b1.eyo.annotated_p = 'fou+bar'
    chai.assert(b1.eyo.annotated_p === 'fou+bar', 'MISSED ANNOTATION')
    ...
    eYo.Test.assert_code(b1, 'x:fou+bar=')
    b2.eyotarget_p = 'xxx'
    chai.assert(b2.eyotarget_p === 'xxx', 'MISSED 2')
    eYo.Test.assert_code(b1, 'xxx:fou+bar=')
    var b3 =  eYo.DelegateSvg.newBlockReady(Blockly.mainWorkspace, `<x eyo="a_expr" operator="+" xmlns="urn:edython:0.2" xmlns:eyo="urn:edython:0.2"><x eyo="identifier" name="abc" slot="lhs"></x><x eyo="identifier" name="bcd" slot="rhs"></x></x>`)
    eYo.Test.assert_code(b3, 'abc+bcd')
    b1.eyo.annotated_s.connect(b3)
    chai.assert(b1.eyo.annotated_t === b3, 'MISSED C8N')
    chai.assert(!b1.eyo.annotated_s.bindField.visible_, 'UNEXPECTED VISIBLE')
    eYo.Test.assert_code(b1, 'xxx:abc+bcd=')
    ...
    eYo.Test.assert_code(b3, 'Z:abcd+cdef')
    eYo.Test.assert_block(b3, 'identifier_annotated')
    // it's a unique target
    chai.assert(b1.eyo.target_t.inputList[0].eyo.connect(b3), 'MISSED C8N 4')
    eYo.Test.assert_block(b1, 'annotated_assignment_stmt')
    eYo.Test.assert_input_length(b1.eyo.target_t, 1, `MISSED C8N 5`)
    var eyo = b1.eyo.target_s.unwrappedTarget
    chai.assert(eyo === b3.eyo, 'MISSED C8N 7')
    console.error(b1.eyo.toLinearString)
    eYo.Test.assert_code(b3, 'xxx:abcd+cdef')
    b3.eyo.name_t.dispose() // orphan recovery: the old b2 block has been reconnected into b3
    eYo.Test.assert_code(b1, 'Z:abcd+cdef=')
    b1.dispose()
  })
})
...
describe('Copy/Paste with value', function() {
  var f = t => {
    it(`basic ${t}`, function() {
      ...
      chai.assert(b.eyo.value_t.eyo.lastConnect(bb))
      var dom = eYo.Xml.blockToDom(b)
      // console.log(dom)
      chai.assert(dom.tagName.toLowerCase() === 's')
      bb = eYo.DelegateSvg.newBlockReady(Blockly.mainWorkspace, dom)
      eYo.Test.assert_block(b, bb.type)
      eYo.Test.assert_variant(b, bb.eyo.variant_p)
      eYo.Test.assert_comment_variant(b, bb.eyo.comment_variant_p)
      eYo.Test.assert_code(b, bb.eyo.toLinearString)
      b.dispose()
      bb.dispose()
    })
  }
