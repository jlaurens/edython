...
export namespace eYo {

  export var TESTING: boolean = true

  export type id_t = string | symbol
   ...
   * @return {Boolean}
   */
  export let objectHasOwnProperty = (object: any, key: id_t): boolean => {
   ...
   * @return {!Boolean}
   */
  export let isF = (what: any): boolean => {
  ...
  /**
   * | Function 
   * o throw. Trick to throw in an expression.
   * @param {String} [what]
   */
  export let error = (what: any): any => {
    console.error(what)
    return what
  }
  /**
   ...
   * @param {String} [what]
   */
  export let raise = (what): void => {
    throw what
  }
  /**
   * Void function frequently used.
   */
  export let doNothing = function (): void {} // NO SHORTHAND
  /**
   * Identity function frequently used.
   */
  export let doReturn = function (what: any): any {
    return what
  }
  /**
   * Projection function frequently used.
   */
  export let doReturn2nd = function (what: any, $else: any): any {
    return $else
  }
   ...
   * @return {!Boolean}
   */
  export let isDoIt = (what: any): boolean => {
    return what !== doNothing && typeof what === 'function' && !!what.call
    //<<< mochai: eYo.isDoIt
   ...
   * @param {*} what
   */
  export let isStr = (what: any): what is string => {
   ...
   * @param {*} what
   */
  export let isSym = (what: any): what is symbol => {
   ...
   * @param {*} what
   */
  export let isId = (what: any): what is symbol | string => {
   ...
   * @param {*} what
   */
  export let isNA = (what: any): boolean => {
    return what === NA
    //<<< mochai: eYo.isNA
    //... let x
   ...
   * @param {*} what
   */
  export let isBool = (what: any): what is boolean => {
   ...
   * @param {*} what
   */
  export let isNum = (what: any): boolean => {
   ...
   * @param {*} what
   */
  export let isDef = (what: any): boolean => {
  ...
  /**
   * | Function 
   * sed when defining a JS property.
   */
  export let noGetter = (msg?: string | Function): () => any => {
    return (isStr(msg)
      ? function () {
        raise(`Forbidden getter: ${msg}`)
      } : isF(msg)
        ? function () {
          raise(`Forbidden getter ${msg.call(this)}`)
        } : function () {
          raise('Forbidden getter')
        }
    ) as () => any
    //<<< mochai: eYo.noGetter
  ...
  type arrow_any_t = ($: any) => void
  /**
   * | Function 
   * sed when defining a JS property.
   * @param {String|| Function}
   * [msg] - Either a string of a function.
   */
  export let noSetter = (msg?: string | Function): arrow_any_t => {
    return (isStr(msg)
      ? function () {
        raise(`Forbidden setter: ${msg}`)
      } : isF(msg)
        ? function () {
          raise(`Forbidden setter ${msg.call(this)}`)
        } : function () {
          raise('Forbidden setter')
        }
    ) as arrow_any_t
    //<<< mochai: eYo.noSetter
  ...
  export type with_lazy_t = {lazy: string}
   ...
   * @private
   */
  export let descriptorR = (getter: () => any,
      msg?: string | with_lazy_t | boolean,
      configurable?: boolean): PropertyDescriptor => {
    // Expected ordered parameter types : f, s, b
    if (isBool(msg)) {
      isDef(configurable) && raise(`eYo.descriptorR: Unexpected last argument ${configurable}`)
      ;[msg, configurable] = [NA, msg]
    } else if (!isStr(msg)) {
       msg = msg.lazy
    }
    return {
      get: getter,
      set: noSetter(msg as string), // Not catched by TS
   ...
   * @private
   */
  export let descriptorW = (
    setter: ($: any)=> void,
    msg: string | with_lazy_t | boolean,
    configurable: boolean
  ): PropertyDescriptor => {
    if (isBool(msg)) {
      isNA(configurable) || raise(`eYo.descriptorW: Unexpected last argument ${configurable}`)
      ;[msg, configurable] = [eYo.NA, msg]
    } else if (!isStr(msg)) {
       msg = msg.lazy
    }
    return {
      get: noGetter(msg as string),
      set: setter,
      configurable: !!configurable
    }
   ...
   * @private
   */
  export let descriptorNORW = (msg?: boolean | string | with_lazy_t, configurable?: boolean): PropertyDescriptor => {
    if (isBool(msg)) {
      isDef(configurable) && raise(`eYo.descriptorNORW: Unexpected last argument ${configurable}`)
      ;[msg, configurable] = [eYo.NA, msg]
    } else if (!isStr(msg)) {
       msg = msg.lazy
    }
    return {
      get: eYo.noGetter(msg as string), // TS won't guess this
      set: eYo.noSetter(msg as string), // TS won't guess this
      configurable,
    }
   ...
   */
  export let new$$ = function (key: string): symbol {
    //<<< mochai: new$$
    if (objectHasOwnProperty(this.$$, key)) {
   ...
   * @param {String} ...
   */
  export let  make$$ = function (...$: Array<string>): void {
    //<<< mochai: make$$
    for(let key of $) {
      new$$(key)
    }
   ...
   */
  export let mixinRO = (object, props) => {
    [Object.keys, Object.getOwnPropertySymbols].forEach(f => {
      f(props).forEach(key => {
        objectHasOwnProperty(object, key) && raise(`Duplicate keys|symbols are forbidden: ${object}, ${key.toString ? key.toString() : key}`)
        var value = props[key]
        Reflect.defineProperty(
          object,
          key,
          descriptorR(eYo.isF(value) ? value : function () {
            return value
          })
        ) || eYo.raise(`eYo.mixinRO: problem with ${key.toString ? key.toString() : key}`)
      })
    })
    return object
    //<<< mochai: mixinR
   ...
   * @return {*} the destination
   */
  export let mixinFR = (object: any, props: any): any => {
    [Object.keys, Object.getOwnPropertySymbols].forEach(f => {
      f(props).forEach(key => {
        objectHasOwnProperty(object, key) && raise(`Duplicate keys|symbols are forbidden: ${object}, ${key.toString ? key.toString() : key}`)
        let value = props[key]
        ...
        ) || eYo.raise(`eYo.mixinFR: problem with ${key.toString ? key.toString() : key}`)
      })
    })
    return object
    //<<< mochai: mixinFR
   ...
   * @return {*} the destination
   */
  export let provideRO = (dest: any, props: any): any => {
    [Object.keys, Object.getOwnPropertySymbols].forEach(f => {
      f(props).forEach(key => {
        if (!objectHasOwnProperty(dest, key)) {
          let value = props[key]
          let d = descriptorR(isF(value) ? value : {$ () {
            return value
          }}.$)
          let dd = Object.getOwnPropertyDescriptor(props, key)
          d.enumerable = dd.enumerable
          d.configurable = dd.configurable
          Reflect.defineProperty(dest, key, d) || raise(`eYo.provideRO: problem with ${key.toString ? key.toString() : key}`)
        }
      })
    })
    return dest
   ...
   * @return {*} the destination
   */
  export let provideFR = (dest: any, props: any): any => {
    [Object.keys, Object.getOwnPropertySymbols].forEach(f => {
      f(props).forEach(key => {
        if (!objectHasOwnProperty(dest, key)) {
          let value = props[key]
          ...
          d.enumerable = dd.enumerable
          d.configurable = dd.configurable
          Reflect.defineProperty(dest, key, d) || raise(`eYo.provideFR: problem with ${key.toString ? key.toString() : key}`)
        }
      })
    })
    return dest
  ...
  /**
   * | Function 
   * send to disallow sending twice the same message.
   */
  export let neverShot = (msg: string | Function) => {
    return eYo.isStr(msg)
      ? function () {
        raise(`Forbidden call: ${msg}`)
      } : eYo.isF(msg)
        ? function () {
          raise(`Forbidden call ${msg.call(this)}`)
        } : function () {
          raise('Forbidden shot')
        }
  ...
  /**
   * | Function 
   * sed to disallow sending twice the same message.
   */
  export let oneShot = (msg) => {
    let ans = (eYo.isStr(msg)
      ? {$ () {
        ...
        ? {$ () {
          ...
          ? eYo.raise(`eYo.oneShot: Bad argument ${msg}`)
          : {$ () {
...
eYo.mixinFR(eYo._p, {
  ...
  newNS (ns, id, model, getters) {
    ...
    if (ns && !eYo.isaNS(ns)) {
      eYo.isDef(getters) && eYo.raise(`${this.name}/newNS: Unexpected last argument: ${getters}`)
    ...
    if (eYo.isSym(id)) {
      ...
      eYo.isDef(getters) && eYo.raise(`${this.name}/newNS: Unexpected last argument (2): ${getters}`)
    ...
    if (anonymous) {
      ns && key && !eYo.isNA(ns[key]) && eYo.raise(`${ns.name}[${key}] already exists.`)
    }
    //... var ns = eYo.newNS()
    ...
    if (key) {
      if (!anonymous && ns) {
        eYo.objectHasOwnProperty(ns, key) && eYo.raise(`${this.name}/newNS: already property (${key})`)
  ...
  require (name) {
    ...
    name.split('.').forEach(k => {
      ...
      eYo.raise(`Missing required ${name}`)
    })
    //... var key = eYo.genUID(eYo.IDENT)
    //... eYo.provide(`${key}.foo`)
    //... eYo.require(`${key}`)
