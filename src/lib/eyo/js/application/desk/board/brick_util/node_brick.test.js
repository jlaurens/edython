...
//       eYo.test.setItUp()
//       var err_ret = {}
//       var n = eYo.py.parser.parseString('abc', g, eYo.py.file_input, err_ret)
...
//   })
  ...
  x.forEach(k => ra_pass_comment.push(k))
  x.forEach(k => x.forEach(l => ra_pass_comment.push(`${k}\n${l}`)))
  x.forEach(k => x.forEach(l => x.forEach(m => ra_pass_comment.push(`${k}\n${l}\n${m}`))))

  ra_pass_comment.reverse()
  ra_pass_comment.length -= 21
  ra_pass_comment.reverse()
  ra_pass_comment.length = 1

  ra_test('pass+comment', ra_pass_comment)
...
//   ]
//   /* ra_test('chai.assert', ra_chai_assert) */
  var ra_with = [
    "with open('x'): pass\n",
    "with open('x') as f: pass\n",
...
//       chai.assert(eYo.py.parser.parseString)
//       var err_ret = {}
//       var n = eYo.py.parser.parseString(src, g, eYo.py.file_input, err_ret)
