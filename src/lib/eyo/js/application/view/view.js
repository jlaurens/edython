 ...
 */
eYo.dlgt.BaseC3s_p.viewEnhanced = function () {
  //<<< mochai: viewEnhanced
   ...
   */
  eYo.mixinFR(this._p, {
    //<<< mochai: eYo.dlgt.BaseC3s_p.viewShortcuts
    viewShortcuts () {
  ...
  eYo.mixinFR(this._p, {
    ...
    c3sDispose (instance, ...$) {
      //<<< mochai: c3sDispose
      //... chai.expect(C3s[eYo.$].c3sDispose).eyo_F
 ...
 * @name{eYo.view.BaseC3s}
 * @constructor
 */
eYo.view.makeBaseC3s(true, {
  /**
   * Initializer.
   */
  init(builtin, ...$) {
    ...
    [eYo.model.ANY]: {
      ...
      [eYo.model.VALIDATE] (before, p) {  // eslint-disable-line
        if (eYo.isStr(before)) {
          ...
          if (eYo.isSubclass(v, eYo.view.BaseC3s)) {
            return {
              C3s: v,
              shared: true,
            }
        ...
        } else if (eYo.isSubclass(before, eYo.view.BaseC3s)) {
          return {
            C3s: before,
            shared: true,
          }
...

{
  ...
  let mf = eYo.view.BaseC3s[eYo.$].modelFormat
...
eYo.View[eYo.$].methodsMerge({
  ...
  //... let driver = {
  //...   prepareUI (instance, ...$) {
  //...     eYo.flag.push(1, instance.eyo$.model.bar, ...$)
  //...   },
  //...   do_initUI (instance, ...$) {
  //...     eYo.flag.push(2, instance.eyo$.model.bar, ...$)
  //...   },
  //...   do_disposeUI (instance, ...$) {
  //...     eYo.flag.push(3, instance.eyo$.model.bar, ...$)
  //...   },
  //... }
  /**
   * Prepare the ui.
  ...
  prepareUI (...$) {
    ...
    //... V.viewForEach(V => eYo.flag.push(V[eYo.$].model.bar))
    //... eYo.flag.expect(567)
    //... V.drvr = driver
    //... V.viewForEach(V => {
    //...   V.drvr = driver
    //... })
    //... V.prepareUI(8, 9)
    //... eYo.flag.expect(1489158916891789)
    //>>>
  },
  /**
   * Make the ui.
  ...
  do_initUI (...$) {
    ...
    //... })
    //... V.do_initUI(8, 9)
    //... eYo.flag.expect(2489258926892789)
    //>>>
  },
  /**
   * Make the ui.
  ...
  do_disposeUI (...$) {
    ...
    //... })
    //... V.do_disposeUI(8, 9)
    //... eYo.flag.expect(3489358936893789)
    //>>>
  },
  /**
   * Dispose of the ui.
