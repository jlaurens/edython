 ...
 * @param {String} str
 */
eYo.py.parser.Scan_p.reset = function (str, start) {
  this.str = str
...
/**
 */
eYo.py.parser.Scan_p.nextToken = function () {
  var d
  ...
  var new_Token = (type, subtype = null) => {
    var token = eYo.py.node.new(this, type, subtype)
  ...
  var new_Error = (subtype) => {
    var token = new_Token(eYo.py.tkn.ERRORTOKEN, subtype)
    this.error = token
    ++ this.errorCount
    return token
  }
  ...
  var new_Indent = (col, altcol) => {
    var token = new_Token(eYo.py.tkn.INDENT)
    token.col = col
    token.altcol = altcol
  ...
  var new_Dedent = (indent) => {
    var tkn = new_Token(eYo.py.tkn.DEDENT)
    if (indent) {
      tkn.indent = indent
      indent.dedent = tkn
    }
 ...
 /**
   ...
   */
  var new_NAME_ = () => {
    var t = new_Token(eYo.py.tkn.NAME)
    if (this.c >= 128) {
      forward()
 ...
 /**
  ...
  var new_EOF = () => {
    ...
    if ((indent = this.indent_stack.pop())) {
      ...
      if (this.last.type != eYo.py.tkn.NEWLINE) {
        new_Token(eYo.py.tkn.NEWLINE)
      }
      do {
        new_Dedent(indent)
      } while ((indent = this.indent_stack.pop()))
      this.at_eof = true
      return new_Token(this.start_type === eYo.py.single_input ? eYo.py.tkn.NEWLINE : eYo.py.tkn.ENDMARKER)
    }
    if (!this.at_eof) {
      // ensure there is a newline
      if (!this.last || this.last.type != eYo.py.tkn.NEWLINE) {
        new_Token(eYo.py.tkn.NEWLINE)
      }
      this.at_eof = true
      return new_Token(this.start_type === eYo.py.single_input ? eYo.py.tkn.NEWLINE : eYo.py.tkn.ENDMARKER)
  ...
  var new_COMMENT = () => {
    if (this.end > this.start) {
      ...
      return new_Token(eYo.py.tkn.COMMENT)
    }
  }
  ...
  var new_box_literal = (x, X, reader, subtype, MSG) => {
    if (scan(x) || scan(X)) {
      ...
      while (true) {
        ...
        return new_Token(eYo.py.tkn.NUMBER, subtype)
      }
    }
  }
    ...
    if (scan('e') || scan('e')) {
      ...
      if (scan('+') || scan('-')) {
        if ((d = scan_digitpart()) && d < this.end) {
          new_Error(eYo.py.E.INVALID_EXPONENT)
        }
  ...
  var new_KeyWord = (kw, start = 0) => {
    if (scan_w(kw, start)) {
      ...
      if (m) {
        ...
        new_Token(eYo.py.tkn.NAME, kw).be_keyword()
        read_space()
      }
      return true
    }
 ...
 */
  var scan_Comment = col => {
    ...
    if (scan('#')) {
      ...
      col && (do_space())
      var m = exec(eYo.py.XRE.type_comment)
      if (m) {
        forward(m[0].length)
 ...
 /**
  ...
  var anchor_letter_quote = () => {
    ...
    if (quote) {
      ...
      new_Token(eYo.py.tkn.STRING).first_lineno = this.first_lineno
      if (end) {
        new_EOF()
      }
      return true
            ...
            while (true) {
              if (this.indent_stack.length > 1) {
                ...
                if (col === indent.col) {
                  ...
                  new_Error(eYo.py.E.DEDENT_AUGMENTED)
    ...
    /* Set start of current token */
      ...
      */
      ...
      if ((m = exec(eYo.py.XRE.prefix))) {
        ...
        return shift()
      } else if ((m = exec(eYo.py.XRE.id_start))) {
      ...
      if (scan('.')) {
        if ((d = scan_digitpart())) {
          if (d < this.end) {
            new_Error(eYo.py.E.NO_TRAILING_UNDERSCORE)
          }
          new_exponent_j() || new_Token(eYo.py.tkn.NUMBER, 'floatnumber')
        } else {
          if (scan('.')) {
            if (scan('.')) {
              new_Token(eYo.py.tkn.ELLIPSIS)
              return shift()
            }
            backward()
          }
      ...
      if (scan('0')) {
        ...
        }, 'hexinteger', eYo.py.E.INVALID_HEXADECIMAL)
          || new_box_literal('o', 'O', () => {
            if ((this.c && this.c >= '0' && this.c <= '7')) {
              forward()
              return true
            }
          }, 'octalinteger', eYo.py.E.INVALID_OCTAL_INTEGER)
          || new_box_literal('b', 'B', () => {
            if(this.c === '0' || this.c === '1') {
              forward()
              return true
            }
          }, 'bininteger', eYo.py.E.INVALID_BINARY_INTEGER)) {
          return shift()
        }
        ...
        if (scan('.')) {
          ...
          if (nonzero) {
            ...
            new_Error(eYo.py.E.NO_LEADING_ZERO)
            // return syntaxerror(tok,
            //   "leading zeros in decimal integer " +
            //   "literals are not permitted " +
            //   "use an 0o prefix for octal integers")
          }
          new_Token(eYo.py.tkn.NUMBER, 'integer')
        }
        return shift()
      }
      else if ((d = scan_digitpart())) {
        ...
        new_exponent_j() || new_Token(eYo.py.tkn.NUMBER, this.subtype)
        return shift()
      }
      ...
      if (scan('\\')) {
        if (scan('\r')) {
          ...
          this.forward
          new_Error(eYo.py.E.UNEXPECTED_ESCAPE)
