describe('Tokenizer', function() {
  ...
  describe('Scan(XRegExp)', function() {
    ...
    it(key, function() {
      var str = 'a'
      var m = XRegExp.exec(str, eYo.py.XRE.id_start, 0, true)
      assert(m && (m[0] === str), `id_start ${str}`)
      str = 'ᢅ'
      m = XRegExp.exec(str, eYo.py.XRE.id_start, 0, true)
      assert(m && (m[0] === str), `id_start ${str}`)
      str = '·'
      m = XRegExp.exec(str, eYo.py.XRE.id_continue, 0, true)
      assert(m && (m[0] === str), `id_continue ${str}`)
      str = 'ᢅ·'
      m = XRegExp.exec(str, eYo.py.XRE.id_continue, 0, true)
      assert(m && (m[0] === str), `id_continue ${str}`)
      str = 'br'
      m = XRegExp.exec(str, eYo.py.XRE.prefix, 0, true)
  ...
  Tester.prototype.test = function (do_it = null) {
    var scan = this.scan = new eYo.py.parser.Scan()
    ...
    while ((key = this.nextKey)) {
      ...
      assert(scan.last.type === eYo.parse[key], `${scan.last.type} === ${eYo.parse[key]}`)
  ...
  Tester.testSrc = function (str) {
    ...
    assert(scan.last.type === eYo.py.tkn.ENDMARKER, `${scan.last.type} === eYo.py.tkn.ENDMARKER`)
  }
  ...
  describe('Scan(COMMENT)', function() {
    ...
    it('TYPE_COMMENT', function() {
      ...
      tester.test()
      assert(tester.scan.last.start_string === eYo.NA, 'Unexpected comment')
    });
