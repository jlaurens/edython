describe('Test', function() {
  ...
  it('test', function() {
    assert(eYo.py.parser.parseString)
    var err_ret = {}
    var n = eYo.py.parser.parseString('', g, eYo.py.single_input, err_ret)
    ...
    describe(name, function() {
      str_s.forEach((str) => {
        var f = (() => {
          return function() {
            var err_ret = {}
            var n = eYo.py.parser.parseString(str, g, eYo.py.single_input, err_ret)
            // eYo.py.node.listTree(n)
            n_test(n, eYo.py.single_input, null)
            assert(err_ret.error === eYo.e.DONE, `ERROR: ${err_ret.error}`)
            // n_test(n, eYo.py.single_input, null)
            // n = n.n_child[0]
            // n_test(n, eYo.py.tkn.ENDMARKER, '')
          }
        })()
        it(str, f)
      })
