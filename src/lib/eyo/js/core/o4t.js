...
;(() => {
   ...
   */
  _p.registerInit = function (k, model) {
    if (eYo.isNA(model)) {
      console.error('BREAK HERE!')
    }
   ...
   */
  _p.registerDispose = function (k, model) {
   ...
   * f any.
   */
  _p.CONSTDeclare = function (k, model = {}) {
    var f = (m) => {
      if (m.unique) {
        return {
          value: eYo.UNIK(),
   ...
   * f any.
   */
  _p.valuedDeclare_ = function (k, model) {
   ...
   * @param {Array<String>} names names of the link to add
   */
  _p.valuedDeclare = function (model) {
    if (model.forEach) {
      model.forEach(k => {
   ...
   */
  _p.valuedClear_ = function (object) {
   ...
   */
  _p.ownedDeclare_ = function (k, model = {}) {
    eYo.parameterAssert(!this.props__.has(k))
    this.owned_.add(k)
    let _p = this.C9r_p
    var k_ = k + '_'
   ...
   */
  _p.ownedDispose_ = function (object, ...params) {
    this.ownedForEach(k => {
      var k_ = k + '_'
      var k__ = k + '__'
      var x = object[k__]
   ...
   */
  _p.cachedDeclare_ = function (k, model) {
    eYo.parameterAssert(!this.props__.has(k))
    this.cached_.add(k)
   ...
   */
  _p.cachedDeclare = function (many) {
   ...
   */
  _p.computedDeclare = function (models) {
