...
/**
 * @name {eYo.shape.BaseC3s}
 * @constructor
 */
eYo.shape.makeBaseC3s({
  init () {
 ...
 * begin
 */
eYo.shape.BaseC3s_p.begin = function () {
  this.steps = []
  this.cursor.set()
  this.dc = this.dl = 0
}
 ...
 * z
 */
eYo.shape.BaseC3s_p.z = function () {
  this.steps.push('z')
}
 ...
 */
eYo.shape.BaseC3s_p.end = function (noClose = false) {
 ...
 * @param {Number} x
 */
eYo.shape.BaseC3s_p.format = function (x) {
 ...
 * end
 */
eYo.shape.BaseC3s_p.push = function () {
  var i
 ...
 * @param {*?} l
 */
eYo.shape.BaseC3s_p.m = function (is_brick, c = 0, l = 0) {
  if (is_brick === true) {
 ...
 * @param {*?} l
 */
eYo.shape.BaseC3s_p.l = function (is_brick, c = 0, l = 0) {
  if (is_brick === true) {
    if (eYo.isDef(c.x) && eYo.isDef(c.y)) {
      l = c.y
      c = c.x
 ...
 * @param {*} c
 */
eYo.shape.BaseC3s_p.h = function (is_brick = false, c = 0) {
  if (is_brick === true) {
    if (c) {
 ...
 * @param {*} l
 */
eYo.shape.BaseC3s_p.v = function (is_brick, l) {
  if (is_brick === true) {
    if (l) {
 ...
 */
eYo.shape.BaseC3s_p.quarter_circle = function (r, clockwise, part) {
  if (r === null) {
 ...
 * @param {Boolean} [clockwise]  Drawing direction.
 */
eYo.shape.BaseC3s_p.half_circle = function (r, clockwise, part) {
 ...
 * @param {Boolean} down
 */
eYo.shape.BaseC3s_p.arc = function (h, r = true, left = true, down = true) {
  if (r === true || r === false) {
    down = left
    left = r
    r = this.expr_radius
...
eYo.shape.newWithBrick = function(brick) {
  return new eYo.shape.BaseC3s().initWithBrick(brick)
}
 ...
 * @param {eYo.brick!} brick  Brick
 */
eYo.shape.BaseC3s_p.initWithBrick = (() => {
  /**
   * Inits a shape with the given brick.
   * @param {eYo.brick!} brick  Brick
   * @return {!Object} The receiver.
...
eYo.shape.newWithMagnet = function(magnet) {
  return new eYo.shape.BaseC3s().initWithMagnet(magnet)
}
 ...
 * @return {!Object} The receiver.
 */
eYo.shape.BaseC3s_p.initWithMagnet = function(magnet, opt) {
  var dd = this.caret_extra
  if (magnet) {
  ...
  if (magnet && opt && opt.hilight) {
    if (magnet.isSlot) {
      if (m4t.targetBrick) {
        this.push(magnet.targetBrick.ui.drvr.pathValueDef_())
      } else if (!brick.disabled) {
        this.initWithMagnet(magnet, {absolute: true})
      }
    } else if (magnet.isOut) {
      this.push(brick.ui.drvr.pathValueDef_(magnet))
 ...
 * @return {!Object} The receiver.
 */
eYo.shape.BaseC3s_p.initForPlay = function (cursor, isContour) {
  this.begin()
  var lh = eYo.geom.Y / 2
  var ratio = 1.618
  var blh = lh * ratio
