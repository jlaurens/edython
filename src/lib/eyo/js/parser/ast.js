 ...
 */
/**
 * @fileoverview cpython's ast.c counterparts.
...
#define NEW_IDENTIFIER(n) new_identifier(n.n_str, c)
...
ast_error(struct compiling *c, const node *n, const char *errmsg, ...)
{
    ...
    loc = PyErr_ProgramTextObject(c->c_filename, n.n_lineno);
    if (!loc) {
        Py_INCREF(Py_None);
        loc = Py_None;
    }
    tmp = Py_BuildValue("(OiiN)", c->c_filename, n.n_lineno, n->n_col_offset + 1, loc);
    if (!tmp) {
        Py_DECREF(errstr);
        return 0;
    }
...
#define NEW_TYPE_COMMENT(n) new_type_comment(n.n_str, c)
                     ...
                     PyObject *filename, PyArena *arena)
{
    ...
    switch (n.n_type) {
        case file_input:
            ...
            for (i = 0; i < num; i++) {
                type_ignore_ty ti = TypeIgnore(ch.n_child[i].n_lineno, arena);
                if (!ti)
                   goto out;
        ...
        case func_type_input:
            ...
            if (n.n_child[1].n_type == typelist) {
                ...
                for (i = 0; i < NCH(ch); i++) {
                    if (ch.n_child[i].n_type == test) {
                        arg = ast_for_expr(&c, ch.n_child[i]);
                        if (!arg)
                            goto out;
            ...
            ret = ast_for_expr(&c, n.n_child[NCH(n] - 1));
            if (!ret)
                goto out;
...
seq_for_testlist(struct compiling *c, const node *n)
{
    ...
    *-/
    ...
    expr_ty expression;
    int i;
    assert(n.n_type == testlist || n.n_type == testlist_star_expr || n.n_type == testlist_comp);
    ...
    for (i = 0; i < NCH(n); i += 2) {
        ...
        assert(ch.n_type == test || ch.n_type == test_nocond || ch.n_type == star_expr || ch.n_type == namedexpr_test);
...
ast_for_arg(struct compiling *c, const node *n)
{
    ...
    assert(n.n_type == tfpdef || n.n_type == vfpdef);
    ch = n.n_child[0];
    ...
    if (NCH(n) == 3 && n.n_child[1].n_type == COLON) {
        annotation = ast_for_expr(c, n.n_child[2]);
        if (!annotation)
            return NULL;
    }

    ret = arg(name, annotation, NULL, n.n_lineno, n->n_col_offset,
        ...
        switch (ch.n_type) {
            ...
            case tfpdef:
                ...
                if (forbidden_name(c, argname, ch, 0))
                    goto error;
                arg = arg(argname, annotation, NULL, ch.n_lineno, ch->n_col_offset,
...
ast_for_arguments(struct compiling *c, const node *n)
{
    ...
    *-/
    ...
    if (n.n_type == parameters) {
        if (NCH(n) == 2) /* () as argument list *-/
            return arguments(NULL, NULL, NULL, NULL, NULL, NULL, c->c_arena);
        n = n.n_child[1];
    }
    assert(n.n_type == typedargslist || n.n_type == varargslist);
    ...
    *-/
    for (i = 0; i < NCH(n); i++) {
        ...
        if (ch.n_type == STAR) {
            ...
            if (i < NCH(n) && /* skip argument following star *-/
                (n.n_child[i].n_type == tfpdef ||
                 n.n_child[i].n_type == vfpdef)) {
                i++;
            }
            break;
        }
        if (ch.n_type == DOUBLESTAR) break;
        if (ch.n_type == vfpdef || ch.n_type == tfpdef) nposargs++;
        if (ch.n_type == EQUAL) nposdefaults++;
    }
    ...
    for ( ; i < NCH(n); ++i) {
        ch = n.n_child[i];
        if (ch.n_type == DOUBLESTAR) break;
        if (ch.n_type == tfpdef || ch.n_type == vfpdef) nkwonlyargs++;
    }
    ...
    *-/
    ...
    while (i < NCH(n)) {
        ...
        switch (ch.n_type) {
            case tfpdef:
            case vfpdef:
                /* XXX Need to worry about checking if n.n_child[i+1].n_type is
                ...
                i += 1; /* the name *-/
                if (i < NCH(n) && n.n_child[i].n_type == COMMA)
                    i += 1; /* the comma, if present *-/
                break;
            case STAR:
                if (i+1 >= NCH(n) ||
                    (i+2 == NCH(n) && (n.n_child[i+1].n_type == COMMA
                                       || n.n_child[i+1].n_type == TYPE_COMMENT))) {
                    ast_error(c, n.n_child[i],
                              "named arguments must follow bare *");
                    return NULL;
                }
                ch = n.n_child[i+1];  /* tfpdef or COMMA *-/
                if (ch.n_type == COMMA) {
                    int res = 0;
                    i += 2; /* now follows keyword only arguments *-/

                    if (i < NCH(n) && n.n_child[i].n_type == TYPE_COMMENT) {
                        ast_error(c, n.n_child[i],
                ...
                if (i < NCH(n) && n.n_child[i].n_type == TYPE_COMMENT) {
                        vararg->type_comment = NEW_TYPE_COMMENT(n.n_child[i]);
                        if (!vararg->type_comment)
                            return NULL;
                        i += 1;
                    }

                    if (i < NCH(n) && (n.n_child[i].n_type == tfpdef
                                    || n.n_child[i].n_type == vfpdef)) {
                        int res = 0;
                        res = handle_keywordonly_args(c, n, i,
                                                      kwonlyargs, kwdefaults);
                ...
                break;
            case DOUBLESTAR:
                ch = n.n_child[i+1];  /* tfpdef *-/
                assert(ch.n_type == tfpdef || ch.n_type == vfpdef);
...
ast_for_decorator(struct compiling *c, const node *n)
{
    ...
    if (NCH(n) == 3) { /* No arguments *-/
        ...
        d = Call(name_expr, NULL, NULL, n.n_lineno,
        ...
        d = ast_for_call(c, n.n_child[3], name_expr, n.n_child[2], n.n_child[4]);
        if (!d)
            return NULL;
        name_expr = NULL;
    }
                     ...
                     asdl_seq *decorator_seq, bool is_async)
{
    ...
    const node * const n = is_async ? n0.n_child[1] : n0;
    identifier name;
    arguments_ty args;
    asdl_seq *body;
    ...
    if (!args)
        return NULL;
    if (n.n_child[name_i+2].n_type == RARROW) {
    ...
    if (is_async)
        return AsyncFunctionDef(name, args, body, decorator_seq, returns, type_comment,
                                n0.n_lineno, n0->n_col_offset, end_lineno, end_col_offset, c->c_arena);
    else
        return FunctionDef(name, args, body, decorator_seq, returns, type_comment,
                           n.n_lineno, n->n_col_offset, end_lineno, end_col_offset, c->c_arena);
}
...
ast_for_decorated(struct compiling *c, const node *n)
{
    ...
    assert(n.n_child[1].n_type == funcdef ||
           n.n_child[1].n_type == async_funcdef ||
           n.n_child[1].n_type == classdef);

    if (n.n_child[1].n_type == funcdef) {
      thing = ast_for_funcdef(c, n.n_child[1], decorator_seq);
    } else if (n.n_child[1].n_type == classdef) {
      thing = ast_for_classdef(c, n.n_child[1], decorator_seq);
    } else if (n.n_child[1].n_type == async_funcdef) {
      thing = ast_for_async_funcdef(c, n.n_child[1], decorator_seq);
    }
    return thing;
}
...
ast_for_namedexpr(struct compiling *c, const node *n)
{
    ...
    *-/
    ...
    target = ast_for_expr(c, n.n_child[0]);
    if (!target)
        return NULL;
    ...
    return NamedExpr(target, value, n.n_lineno, n->n_col_offset, n->n_end_lineno,
...
ast_for_lambdef(struct compiling *c, const node *n)
{
    ...
    return Lambda(args, expression, n.n_lineno, n->n_col_offset,
                  n->n_end_lineno, n->n_end_col_offset, c->c_arena);
}
...
ast_for_ifexpr(struct compiling *c, const node *n)
{
    ...
    if (!orelse)
        return NULL;
    return IfExp(expression, body, orelse, n.n_lineno, n->n_col_offset,
                 n->n_end_lineno, n->n_end_col_offset,
                 c->c_arena);
}
...
ast_for_comprehension(struct compiling *c, const node *n)
{
    ...
    for (i = 0; i < n_fors; i++) {
        ...
        if (!t)
            return NULL;
        expression = ast_for_expr(c, sync_n.n_child[3]);
        if (!expression)
            return NULL;
...
ast_for_itercomp(struct compiling *c, const node *n, int type)
{
    ...
    if (type == COMP_GENEXP)
        return GeneratorExp(elt, comps, n.n_lineno, n->n_col_offset,
                            n->n_end_lineno, n->n_end_col_offset, c->c_arena);
    else if (type == COMP_LISTCOMP)
        return ListComp(elt, comps, n.n_lineno, n->n_col_offset,
                        n->n_end_lineno, n->n_end_col_offset, c->c_arena);
    else if (type == COMP_SETCOMP)
        return SetComp(elt, comps, n.n_lineno, n->n_col_offset,
                       n->n_end_lineno, n->n_end_col_offset, c->c_arena);
    else
        /* Should never happen *-/
        return NULL;
...
{
    expr_ty expression;
    if (n.n_child[*i].n_type == DOUBLESTAR) {
        assert(NCH(n) - *i >= 2);
        ...
        *i += 2;
        ...
        expression = ast_for_expr(c, n.n_child[*i]);
        if (!expression)
            return 0;
        *key = expression;
...
ast_for_dictcomp(struct compiling *c, const node *n)
{
    ...
    comps = ast_for_comprehension(c, n.n_child[i]);
    if (!comps)
        return NULL;

    return DictComp(key, value, comps, n.n_lineno, n->n_col_offset,
                    n->n_end_lineno, n->n_end_col_offset, c->c_arena);
}
...
ast_for_dictdisplay(struct compiling *c, const node *n)
{
    ...
    keys->size = j;
    values->size = j;
    return Dict(keys, values, n.n_lineno, n->n_col_offset,
                n->n_end_lineno, n->n_end_col_offset, c->c_arena);
}
...
{
    assert(n.n_type == (testlist_comp) || n.n_type == (argument));
...
{
    assert(n.n_type == (dictorsetmaker));
...
ast_for_setdisplay(struct compiling *c, const node *n)
{
    ...
    return Set(elts, n.n_lineno, n->n_col_offset,
               n->n_end_lineno, n->n_end_col_offset, c->c_arena);
}
...
ast_for_atom(struct compiling *c, const node *n)
{
    ...
    *-/
    ...
    case NAME: {
        PyObject *name;
        const char *s = ch.n_str;
        size_t len = strlen(s);
        if (len >= 4 && len <= 5) {
            if (!strcmp(s, "None"))
                return Constant(Py_None, n.n_lineno, n->n_col_offset,
                                n->n_end_lineno, n->n_end_col_offset, c->c_arena);
            if (!strcmp(s, "True"))
                return Constant(Py_True, n.n_lineno, n->n_col_offset,
                                n->n_end_lineno, n->n_end_col_offset, c->c_arena);
            if (!strcmp(s, "False"))
                return Constant(Py_False, n.n_lineno, n->n_col_offset,
                                n->n_end_lineno, n->n_end_col_offset, c->c_arena);
        }
        ...
        return Name(name, Load, n.n_lineno, n->n_col_offset,
                    n->n_end_lineno, n->n_end_col_offset, c->c_arena);
    }
    case STRING: {
    ...
    case NUMBER: {
        ...
        return Constant(pynum, n.n_lineno, n->n_col_offset,
                        n->n_end_lineno, n->n_end_col_offset, c->c_arena);
    }
    case ELLIPSIS: /* Ellipsis *-/
        return Constant(Py_Ellipsis, n.n_lineno, n->n_col_offset,
                        n->n_end_lineno, n->n_end_col_offset, c->c_arena);
    case LPAR: /* some parenthesized expressions *-/
        ch = n.n_child[1];

        if (ch.n_type == RPAR)
            return Tuple(NULL, Load, n.n_lineno, n->n_col_offset,
                         n->n_end_lineno, n->n_end_col_offset, c->c_arena);

        if (ch.n_type == yield_expr)
        ...
        if (ch.n_child[1].n_type == comp_for) {
    ...
    case LSQB: /* list (or list comprehension) *-/
        ...
        if (NCH(ch) == 1 || ch.n_child[1].n_type == COMMA) {
            ...
            return List(elts, Load, n.n_lineno, n->n_col_offset,
                        n->n_end_lineno, n->n_end_col_offset, c->c_arena);
        }
        else {
    ...
    case LBRACE: {
        ...
        if (ch.n_type == RBRACE) {
            /* It's an empty dict. *-/
            return Dict(NULL, NULL, n.n_lineno, n->n_col_offset,
                        n->n_end_lineno, n->n_end_col_offset, c->c_arena);
        }
        else {
            int is_dict = (ch.n_child[0].n_type == DOUBLESTAR);
            if (NCH(ch) == 1 ||
                    (NCH(ch) > 1 &&
                     ch.n_child[1].n_type == COMMA)) {
                /* It's a set display. *-/
                res = ast_for_setdisplay(c, ch);
            }
            else if (NCH(ch) > 1 &&
                    ch.n_child[1].n_type == comp_for) {
...
ast_for_binop(struct compiling *c, const node *n)
{
    ...
    *-/
    ...
    result = BinOp(expr1, newoperator, expr2, n.n_lineno, n->n_col_offset,
                   n.n_child[2]->n_end_lineno, n.n_child[2]->n_end_col_offset,
                   c->c_arena);
    if (!result)
        return NULL;
    ...
    for (i = 1; i < nops; i++) {
        ...
        tmp_result = BinOp(result, newoperator, tmp,
                           next_oper.n_lineno, next_oper->n_col_offset,
...
ast_for_trailer(struct compiling *c, const node *n, expr_ty left_expr)
{
    ...
    if (n.n_child[0].n_type == LPAR) {
        if (NCH(n) == 2)
            ...
            return ast_for_call(c, n.n_child[1], left_expr, n.n_child[0], n.n_child[2]);
    }
    else if (n.n_child[0].n_type == DOT) {
        PyObject *attr_id = NEW_IDENTIFIER(n.n_child[1]);
        if (!attr_id)
            return NULL;
        ...
        if (NCH(n) == 1) {
            ...
            if (!slc)
                return NULL;
            return Subscript(left_expr, slc, Load, n.n_lineno, n->n_col_offset,
            ...
            *-/
            ...
            if (!simple) {
                return Subscript(left_expr, ExtSlice(slices, c->c_arena),
                                 Load, n.n_lineno, n->n_col_offset,
            ...
            e = Tuple(elts, Load, n.n_lineno, n->n_col_offset,
                      n->n_end_lineno, n->n_end_col_offset, c->c_arena);
            if (!e)
                return NULL;
...
ast_for_factor(struct compiling *c, const node *n)
{
    ...
    switch (n.n_child[0].n_type) {
        case PLUS:
            return UnaryOp(UAdd, expression, n.n_lineno, n->n_col_offset,
                           n->n_end_lineno, n->n_end_col_offset,
                           c->c_arena);
        case MINUS:
            return UnaryOp(USub, expression, n.n_lineno, n->n_col_offset,
                           n->n_end_lineno, n->n_end_col_offset,
                           c->c_arena);
        case TILDE:
            return UnaryOp(Invert, expression, n.n_lineno, n->n_col_offset,
                           n->n_end_lineno, n->n_end_col_offset,
                           c->c_arena);
    }
...
ast_for_atom_expr(struct compiling *c, const node *n)
{
    ...
    e = ast_for_atom(c, n.n_child[start]);
    if (!e)
        return NULL;
    if (nch == 1)
        return e;
    if (start && nch == 2) {
        return Await(e, n.n_lineno, n->n_col_offset,
                     n->n_end_lineno, n->n_end_col_offset, c->c_arena);
    }

    for (i = start + 1; i < nch; i++) {
        node *ch = n.n_child[i];
        if (ch.n_type != trailer)
            break;
...
ast_for_power(struct compiling *c, const node *n)
{
    ...
    if (NCH(n) == 1)
        return e;
    if (TYPE(n.n_child[NCH(n] - 1)) == factor) {
        expr_ty f = ast_for_expr(c, n.n_child[NCH(n] - 1));
        if (!f)
            return NULL;
        e = BinOp(e, Pow, f, n.n_lineno, n->n_col_offset,
                  n->n_end_lineno, n->n_end_col_offset, c->c_arena);
    }
    return e;
}
...
ast_for_starred(struct compiling *c, const node *n)
{
    ...
    return Starred(tmp, Load, n.n_lineno, n->n_col_offset,
                   n->n_end_lineno, n->n_end_col_offset, c->c_arena);
}
...
ast_for_expr(struct compiling *c, const node *n)
{
 ...
 loop:
    switch (n.n_type) {
        ...
        case test:
        case test_nocond:
            if (n.n_child[0].n_type == lambdef ||
                n.n_child[0].n_type == lambdef_nocond)
        ...
        case and_test:
            ...
            if (!strcmp(n.n_child[1].n_str, "and"))
                return BoolOp(And, seq, n.n_lineno, n->n_col_offset,
                              n->n_end_lineno, n->n_end_col_offset,
                              c->c_arena);
            assert(!strcmp(n.n_child[1].n_str, "or"));
            return BoolOp(Or, seq, n.n_lineno, n->n_col_offset,
                          n->n_end_lineno, n->n_end_col_offset, c->c_arena);
        case not_test:
            if (NCH(n) == 1) {
                n = n.n_child[0];
                ...
                return UnaryOp(Not, expression, n.n_lineno, n->n_col_offset,
                               n->n_end_lineno, n->n_end_col_offset,
                               c->c_arena);
            }
        case comparison:
            if (NCH(n) == 1) {
                ...
                return Compare(expression, ops, cmps, n.n_lineno, n->n_col_offset,
                               n->n_end_lineno, n->n_end_col_offset, c->c_arena);
            }
            break;
        ...
        case yield_expr: {
            ...
            expr_ty exp = NULL;
            if (NCH(n) > 1)
                an = n.n_child[1]; /* yield_arg *-/
            if (an) {
                en = an.n_child[NCH(an] - 1);
                if (NCH(an) == 2) {
                    is_from = 1;
            ...
            if (is_from)
                return YieldFrom(exp, n.n_lineno, n->n_col_offset,
                                 n->n_end_lineno, n->n_end_col_offset, c->c_arena);
            return Yield(exp, n.n_lineno, n->n_col_offset,
                         n->n_end_lineno, n->n_end_col_offset, c->c_arena);
        }
        case factor:
            if (NCH(n) == 1) {
             ...
             const node *maybegenbeg, const node *closepar)
{
    ...
    *-/
    ...
    for (i = 0; i < NCH(n); i++) {
        node *ch = n.n_child[i];
        if (ch.n_type == argument) {
            if (NCH(ch) == 1)
                nargs++;
            else if (ch.n_child[1].n_type == comp_for) {
                nargs++;
                if (!maybegenbeg) {
            ...
            else if (ch.n_child[0].n_type == STAR)
                nargs++;
            else if (ch.n_child[1].n_type == COLONEQUAL) {
                nargs++;
            }
            else
                /* ch.n_child[0].n_type == DOUBLESTAR or keyword argument *-/
                nkeywords++;
        }
    }
    ...
    for (i = 0; i < NCH(n); i++) {
        ...
        if (ch.n_type == argument) {
            ...
            if (NCH(ch) == 1) {
                ...
                e = ast_for_expr(c, ch.n_child[1]);
                if (!e)
                    return NULL;
                starred = Starred(e, Load, chch.n_lineno,
                        chch->n_col_offset,
            ...
            else if (chch.n_type == DOUBLESTAR) {
                ...
                for (int i = 0; name_tree[i]; i++) {
                    if (expr_node.n_type != name_tree[i])
                        break;
                    if (NCH(expr_node) != 1)
                        break;
                    expr_node = expr_node.n_child[0];
                }
                if (expr_node.n_type != NAME) {
                    ast_error(c, chch,
                              "expression cannot contain assignment, "
                              "perhaps you meant \"==\"?");
                    return NULL;
                }
                key = new_identifier(expr_node.n_str, c);
                if (key == NULL) {
                    return NULL;
                }
...
ast_for_testlist(struct compiling *c, const node* n)
{
    ...
    /* testlist: test (',' test)* [','] *-/
    assert(NCH(n) > 0);
    if (n.n_type == testlist_comp) {
        if (NCH(n) > 1)
            assert(n.n_child[1].n_type != comp_for);
    }
    else {
        assert(n.n_type == testlist ||
               n.n_type == testlist_star_expr);
    }
    if (NCH(n) == 1)
        ...
        if (!tmp)
            return NULL;
        return Tuple(tmp, Load, n.n_lineno, n->n_col_offset,
                     n->n_end_lineno, n->n_end_col_offset, c->c_arena);
    }
}
...
ast_for_expr_stmt(struct compiling *c, const node *n)
{
    ...
    if (num == 1) {
        ...
        return Expr(e, n.n_lineno, n->n_col_offset,
                    n->n_end_lineno, n->n_end_col_offset, c->c_arena);
    }
    else if (n.n_child[1].n_type == augassign) {
        expr_ty expr1, expr2;
        operator_ty newoperator;
        node *ch = n.n_child[0];
        ...
        return AugAssign(expr1, newoperator, expr2, n.n_lineno, n->n_col_offset,
                         n->n_end_lineno, n->n_end_col_offset, c->c_arena);
    }
    else if (n.n_child[1].n_type == annassign) {
        expr_ty expr1, expr2, expr3;
        node *ch = n.n_child[0];
        node *deep, *ann = n.n_child[1];
        int simple = 1;
        ...
        if (NCH(deep) > 0 && deep.n_child[0].n_type == LPAR) {
            simple = 0;
        }
        expr1 = ast_for_testlist(c, ch);
        if (!expr1) {
        ...
        if (value.n_type == testlist_star_expr)
        ...
        if (!expression)
            return NULL;
        if (has_type_comment) {
            type_comment = NEW_TYPE_COMMENT(n.n_child[nch_minus_type]);
            if (!type_comment)
                return NULL;
        }
        else
            type_comment = NULL;
        return Assign(targets, expression, type_comment, n.n_lineno, n->n_col_offset,
                      n->n_end_lineno, n->n_end_col_offset, c->c_arena);
    }
}
...
ast_for_del_stmt(struct compiling *c, const node *n)
{
    ...
    if (!expr_list)
        return NULL;
    return Delete(expr_list, n.n_lineno, n->n_col_offset,
                  n->n_end_lineno, n->n_end_col_offset, c->c_arena);
}
...
ast_for_flow_stmt(struct compiling *c, const node *n)
{
    ...
    *-/
    ...
    switch (ch.n_type) {
        case break_stmt:
            return Break(n.n_lineno, n->n_col_offset,
                         n->n_end_lineno, n->n_end_col_offset, c->c_arena);
        case continue_stmt:
            return Continue(n.n_lineno, n->n_col_offset,
                            n->n_end_lineno, n->n_end_col_offset, c->c_arena);
        case yield_stmt: { /* will reduce to yield_expr *-/
            expr_ty exp = ast_for_expr(c, ch.n_child[0]);
            if (!exp)
                return NULL;
            return Expr(exp, n.n_lineno, n->n_col_offset,
                        n->n_end_lineno, n->n_end_col_offset, c->c_arena);
        }
        case return_stmt:
            if (NCH(ch) == 1)
                return Return(NULL, n.n_lineno, n->n_col_offset,
                              n->n_end_lineno, n->n_end_col_offset, c->c_arena);
            else {
                expr_ty expression = ast_for_testlist(c, ch.n_child[1]);
                if (!expression)
                    return NULL;
                return Return(expression, n.n_lineno, n->n_col_offset,
                              n->n_end_lineno, n->n_end_col_offset, c->c_arena);
            }
        case raise_stmt:
            if (NCH(ch) == 1)
                return Raise(NULL, NULL, n.n_lineno, n->n_col_offset,
                             n->n_end_lineno, n->n_end_col_offset, c->c_arena);
            else if (NCH(ch) >= 2) {
                expr_ty cause = NULL;
                ...
                return Raise(expression, cause, n.n_lineno, n->n_col_offset,
                             n->n_end_lineno, n->n_end_col_offset, c->c_arena);
            }
            /* fall through *-/
        default:
    ...
    if (n.n_type == import_name) {
       ...
       /* Count the number of dots (for relative imports) and check for the
        ...
        for (idx = 1; idx < NCH(n); idx++) {
            if (n.n_child[idx].n_type == dotted_name) {
                mod = alias_for_import_name(c, n.n_child[idx], 0);
                if (!mod)
                    return NULL;
                idx++;
                break;
            } else if (n.n_child[idx].n_type == ELLIPSIS) {
                /* three consecutive dots are tokenized as one ELLIPSIS *-/
                ndots += 3;
                continue;
            } else if (n.n_child[idx].n_type != DOT) {
                break;
            }
            ndots++;
        }
...
ast_for_global_stmt(struct compiling *c, const node *n)
{
    ...
    if (!s)
        return NULL;
    for (i = 1; i < NCH(n); i += 2) {
        name = NEW_IDENTIFIER(n.n_child[i]);
        if (!name)
            return NULL;
        asdl_seq_SET(s, i / 2, name);
    }
    return Global(s, n.n_lineno, n->n_col_offset,
                  n->n_end_lineno, n->n_end_col_offset, c->c_arena);
}
...
ast_for_nonlocal_stmt(struct compiling *c, const node *n)
{
    ...
    return Nonlocal(s, n.n_lineno, n->n_col_offset,
                    n->n_end_lineno, n->n_end_col_offset, c->c_arena);
}
...
ast_for_assert_stmt(struct compiling *c, const node *n)
{
    ...
    if (NCH(n) == 2) {
        ...
        if (!expression)
            return NULL;
        return Assert(expression, NULL, n.n_lineno, n->n_col_offset,
                      n->n_end_lineno, n->n_end_col_offset, c->c_arena);
    }
    else if (NCH(n) == 4) {
        expr_ty expr1, expr2;
        ...
        return Assert(expr1, expr2, n.n_lineno, n->n_col_offset,
                      n->n_end_lineno, n->n_end_col_offset, c->c_arena);
    }
    PyErr_Format(PyExc_SystemError,
...
ast_for_suite(struct compiling *c, const node *n)
{
    ...
    if (!seq)
        return NULL;
    if (n.n_child[0].n_type == simple_stmt) {
        n = n.n_child[0];
...
ast_for_if_stmt(struct compiling *c, const node *n)
{
    ...
    *-/
    ...
    if (NCH(n) == 4) {
        ...
        return If(expression, suite_seq, NULL, n.n_lineno, n->n_col_offset,
    ...
    *-/
    if (s[2] == 's') {
        ...
        return If(expression, seq1, seq2, n.n_lineno, n->n_col_offset,
                  end_lineno, end_col_offset, c->c_arena);
    }
    else if (s[2] == 'i') {
        ...
        if (has_else) {
            ...
            if (!orelse)
                return NULL;
            expression = ast_for_expr(c, n.n_child[NCH(n] - 6));
            if (!expression)
                return NULL;
            suite_seq = ast_for_suite(c, n.n_child[NCH(n] - 4));
            if (!suite_seq)
                return NULL;
            suite_seq2 = ast_for_suite(c, n.n_child[NCH(n] - 1));
            if (!suite_seq2)
                return NULL;
            ...
            asdl_seq_SET(orelse, 0,
                         If(expression, suite_seq, suite_seq2,
                            LINENO(n.n_child[NCH(n] - 6)),
                            n.n_child[NCH(n] - 6)->n_col_offset,
                            end_lineno, end_col_offset, c->c_arena));
        ...
        for (i = 0; i < n_elif; i++) {
            ...
            asdl_seq_SET(newobj, 0,
                         If(expression, suite_seq, orelse,
                            n.n_child[off].n_lineno,
                            n.n_child[off]->n_col_offset,
                            end_lineno, end_col_offset, c->c_arena));
            orelse = newobj;
        }
        expression = ast_for_expr(c, n.n_child[1]);
...
ast_for_while_stmt(struct compiling *c, const node *n)
{
    ...
    if (NCH(n) == 4) {
        ...
        if (!suite_seq)
            return NULL;
        get_last_end_pos(suite_seq, &end_lineno, &end_col_offset);
        return While(expression, suite_seq, NULL, n.n_lineno, n->n_col_offset,
                     end_lineno, end_col_offset, c->c_arena);
    }
    else if (NCH(n) == 7) {
        expr_ty expression;
        ...
        return While(expression, seq1, seq2, n.n_lineno, n->n_col_offset,
                     end_lineno, end_col_offset, c->c_arena);
    }
...
ast_for_for_stmt(struct compiling *c, const node *n0, bool is_async)
{
    ...
    if (has_type_comment) {
        type_comment = NEW_TYPE_COMMENT(n.n_child[5]);
        if (!type_comment)
            return NULL;
    }
    else
...
ast_for_except_clause(struct compiling *c, const node *exc, node *body)
{
    ...
    if (NCH(exc) == 1) {
        ...
        return ExceptHandler(NULL, NULL, suite_seq, exc.n_lineno,
                             exc->n_col_offset,
                             end_lineno, end_col_offset, c->c_arena);
    }
    else if (NCH(exc) == 2) {
        ...
        return ExceptHandler(expression, NULL, suite_seq, exc.n_lineno,
                             exc->n_col_offset,
                             end_lineno, end_col_offset, c->c_arena);
    }
    else if (NCH(exc) == 4) {
        asdl_seq *suite_seq;
        expr_ty expression;
        identifier e = NEW_IDENTIFIER(exc.n_child[3]);
        if (!e)
            return NULL;
        ...
        return ExceptHandler(expression, e, suite_seq, exc.n_lineno,
                             exc->n_col_offset,
                             end_lineno, end_col_offset, c->c_arena);
    }
...
ast_for_try_stmt(struct compiling *c, const node *n)
{
    ...
    return Try(body, handlers, orelse, finally, n.n_lineno, n->n_col_offset,
               end_lineno, end_col_offset, c->c_arena);
}
...
ast_for_with_stmt(struct compiling *c, const node *n0, bool is_async)
{
    ...
    body = ast_for_suite(c, n.n_child[NCH(n] - 1));
    if (!body)
        return NULL;
    get_last_end_pos(body, &end_lineno, &end_col_offset);

    if (has_type_comment) {
        type_comment = NEW_TYPE_COMMENT(n.n_child[NCH(n] - 2));
        if (!type_comment)
            return NULL;
    }
    else
    ...
    if (is_async)
        return AsyncWith(items, body, type_comment, n0.n_lineno, n0->n_col_offset,
                         end_lineno, end_col_offset, c->c_arena);
    else
        return With(items, body, type_comment, n.n_lineno, n->n_col_offset,
                    end_lineno, end_col_offset, c->c_arena);
}
...
ast_for_classdef(struct compiling *c, const node *n, asdl_seq *decorator_seq)
{
    ...
    if (NCH(n) == 4) { /* class NAME ':' suite *-/
        ...
        classname = NEW_IDENTIFIER(n.n_child[1]);
        if (!classname)
            return NULL;
    ...
    if (n.n_child[3].n_type == RPAR) { /* class NAME '(' ')' ':' suite *-/
    ...
    /* build up a fake Call node so we can extract its pieces *-/
    {
        PyObject *dummy_name;
        expr_ty dummy;
        dummy_name = NEW_IDENTIFIER(n.n_child[1]);
        if (!dummy_name)
            return NULL;
        dummy = Name(dummy_name, Load, n.n_lineno, n->n_col_offset,
                     n.n_child[1]->n_end_lineno, n.n_child[1]->n_end_col_offset,
                     c->c_arena);
        call = ast_for_call(c, n.n_child[3], dummy, NULL, n.n_child[4]);
        if (!call)
            return NULL;
    }
    ...
    return ClassDef(classname, call->v.Call.args, call->v.Call.keywords, s,
                    decorator_seq, n.n_lineno, n->n_col_offset,
                    end_lineno, end_col_offset, c->c_arena);
}
...
ast_for_stmt(struct compiling *c, const node *n)
{
    ...
    if (n.n_type == simple_stmt) {
        assert(num_stmts(n) == 1);
        n = n.n_child[0];
    }
    if (n.n_type == small_stmt) {
        n = n.n_child[0];
        ...
        *-/
        switch (n.n_type) {
            ...
            case pass_stmt:
                return Pass(n.n_lineno, n->n_col_offset,
                            n->n_end_lineno, n->n_end_col_offset, c->c_arena);
            case flow_stmt:
...
static void fstring_shift_node_locations(node *n, int lineno, int col_offset)
{
    ...
    for (int i = 0; i < NCH(n); ++i) {
        if (n->n_lineno && n->n_lineno < n.n_child[i]->n_lineno) {
            /* Shifting column offsets unnecessary if there's been newlines. *-/
            col_offset = 0;
        }
        fstring_shift_node_locations(n.n_child[i], lineno, col_offset);
    }
                  ...
                  expr_ty *expression, struct compiling *c, const node *n)
{
    ...
    *expression = FormattedValue(simple_expression, conversion,
                                 format_spec, n.n_lineno, n->n_col_offset,
                                 n->n_end_lineno, n->n_end_col_offset,
                                 c->c_arena);
    if (!*expression)
        return -1;
...
make_str_node_and_del(PyObject **str, struct compiling *c, const node* n)
{
    ...
    *str = NULL;
    ...
    return Constant(s, n.n_lineno, n->n_col_offset,
                    n->n_end_lineno, n->n_end_col_offset, c->c_arena);
}
                     ...
                     const node *n)
{
    ...
    return JoinedStr(seq, n.n_lineno, n->n_col_offset,
                     n->n_end_lineno, n->n_end_col_offset, c->c_arena);
...
parsestrplus(struct compiling *c, const node *n)
{
    ...
    if (bytesmode) {
        ...
        if (PyArena_AddPyObject(c->c_arena, bytes_str) < 0)
            goto error;
        return Constant(bytes_str, n.n_lineno, n->n_col_offset,
                        n->n_end_lineno, n->n_end_col_offset, c->c_arena);
    }
