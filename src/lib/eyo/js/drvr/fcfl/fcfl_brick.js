 ...
 */
eYo.fcfl.newDrvrC9r('Brick')

eYo.brick.C9rBase[eYo.$].p6yMerge({
  ui () {
  ...
  dragging: {
    didChange (before, after) /** @suppress {globalThis} */ {
      this.drvr.draggingSet(this, after)      
    }
  },
  /**
  ...
  bBox: {
    get () {
      return this.rendered && (this.drvr.getBBox(BIconThreeDotsVertical))
    },
  },
  hasSelect: {
    get () {
      return this.rendered && (this.drvr.hasFocus(this))
    },
  },
  visible: {
    /**
     ...
     */
    get () {
      return this.drvr.displayedGet(this)
    },
    /**
    ...
    set (after) { // eslint-disable-line
      this.drvr.displayedSet(this, after)
    }
  },
})
eYo.brick.C9rBase[eYo.$].p6yAliasesMerge({
  'xy': 'where',
})
 ...
 * @param {eYo.brick.C9rBase} brick - The brick to move.
...
eYo.fcfl.Brick._p.newDrawRecorder_ = function (brick, recorder) {
  ...
  io.cursor = eYo.geom.pPoint(0, brick.span.header)
  if (recorder) {
 ...
 * @protected
 */
eYo.drvr.makeForwarder(eYo.brick.C9rBase_p, 'updateShape')

/**
 * Update the shape of the brick.
 * To be subclassed.
 * @param {eYo.brick.C9rBase} brick - The brick of which the shape would need an update
 * @protected
 */
 ...
 * @param {eYo.magnet.C9rBase} targetOldM4t what was previously connected to the new targetConnection
 */
  ...
  var magnet = this.slotSomeMagnet(brick, magnet => {
    if (!magnet.disabled_ && (!magnet.hidden_ || magnet.wrapped_)) {
      if (magnet.isSlot) {
        ...
        if (target) {
          var targetM4t = target.brick.drvr.getMagnetForEvent(target.brick, e)
          if (targetM4t) {
            return targetM4t
          }
          R = new eYo.geom.Rect(
 ...
 * @param {eYo.brick.C9rBase} brick - the brick the driver acts on
 */
eYo.drvr.makeForwarder(eYo.brick.C9rBase_p, 'deleteStyleSet')
 ...
 * Default implementation does nothing.
 * @param {eYo.brick.C9rBase} brick The brick to place.
 */
