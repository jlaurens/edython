...
eYo.expr.literal.prototype.saveData = function (element, opt) { // eslint-disable-line
  eYo.expr.literal.eyo$.C3s_s.saveData.apply(this, arguments)
  if (this.Value_p == '') {
 ...
 * For edython.
 */
eYo.expr.literal[eYo.$newSubC3s]('numberliteral', {
  data: {
    type: {
      all: [
        eYo.t3.expr.unset,
 ...
 * For edython.
 */
eYo.expr.literal[eYo.$newSubC3s]('shortliteral', {
  data: {
    subtype: {
      all: [
      ...
      synchronize /** @this{eYo.data.BaseC3s} */ (after) { // eslint-disable-line
    ...
    delimiter: {
      all: ["'", '"'],
      init: '"',
      didChange (builtin) /** @this{eYo.data.BaseC3s} */ {
        builtin()
        this.brick.value_d.consolidate()
      },
      synchronize (builtin) /** @this{eYo.data.BaseC3s} */ {
        builtin()
    ...
    prefix: {
      ...
      init: '',
      didChange (builtin) /** @suppress {globalThis} */ { /** @this{eYo.data.BaseC3s} */
        builtin()
        this.brick.value_d.consolidate()
      },
      validate (after) { /** @this{eYo.data.BaseC3s} */
        return !eYo.isDef(this.brick.Content_p) || this.brick.validateComponents({
          prefix: after}
        ) ? after : eYo.INVALID
      },
      synchronize (builtin, after) { /** @this{eYo.data.BaseC3s} */ 
    ...
    value: {
      ...
      didChange (builtin, after) /** @suppress {globalThis} */ {/** @this{eYo.data.BaseC3s} */ 
        builtin()
        var b3k = this.brick
...
eYo.expr.shortliteral.prototype.populateContextMenuFirst_ = function (mngr) {
  ...
  eYo.expr.shortliteral.eyo$.C3s_s.populateContextMenuFirst_.call(this, mngr)
  return true
}
 ...
 * For edython.
 */
eYo.expr.shortliteral[eYo.$newSubC3s]('longliteral', {
  data: {
    subtype: {
      all: [
