...
console.log(chalk.keyword('orange').bold('→ .electron-vue/eyo.config.js'))

module.exports = (() => {
  let Config = function (target, dist, env) {
    this.options = {
      web: {
        no_tippy: false,
        brython_debug: false,
  ...
  Config.prototype.getConfig = function () {
    ...
    config.resolve = {
      ...
      extensions: ['.js', '.json', '.css', '.node', '.txt', '.xml', '.eyo', '.py']
    }
  ...
  Config.prototype.enableXRegExp = function (config) {
  ...
  Config.prototype.enableBrython = function (config) {
  ...
  Config.prototype.enableBrythonSources = function (config) {
  ...
  Config.prototype.enableEdython = function (config) {
  ...
  Config.prototype.enableTippy = function (config) {
  ...
  Config.prototype.enableResources = function (config) {
  ...
  Config.prototype.enableVue = function (config) {
    if (this.options.no_vue) {
      return
    }
    config.module.rules = config.module.rules.filter(rule => rule.test !== /\.html$/)
    config.module.rules.push({
      test: /\.js$/,
      enforce: 'pre',
      use: {
    ...
    config.resolve.alias.vue$ = path.join('vue', 'dist', 'vue.esm.js')

  }

  Config.get = function(target, dist, env) {
    var eyo = new Config(target, dist, env)
